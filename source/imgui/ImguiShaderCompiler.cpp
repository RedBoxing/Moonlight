...
CompiledData NOINLINE CreateShaderBinary(GLSLCoutput *compileData, const char *shaderName, bool outputFile = true)
{
    ...
    if (outputFile)
    {
        ...
        char fullPath[0x40] = {};
        createPath(fullPath, "sd:/shaders", shaderName, ".bin");

        R_ABORT_UNLESS(Starlight::FS::writeFile(fullPath, binaryBuffer, binarySize))
    }
...
const char *GetShaderSource(const char *path)
{
    ...
    EXL_ASSERT(Starlight::FS::doesFileExist(path), "Failed to Find File!");
...
CompiledData ImguiShaderCompiler::CompileShader(const char *shaderName)
{
    ...
    char vshPath[0x40] = {};
    createPath(vshPath, "romfs:/shaders/sources", shaderName, "_vsh.glsl");
    char fshPath[0x40] = {};
    createPath(fshPath, "romfs:/shaders/sources", shaderName, "_fsh.glsl");
