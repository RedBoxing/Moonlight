...
namespace ImguiNvnBackend
{
    ...
    void updateInput()
    {
        ...
        {
            if (Starlight::HID::isButtonPressed((nn::hid::NpadButton)nx_k))
                io.AddKeyEvent((ImGuiKey)im_k, true);
            else if (Starlight::HID::isButtonReleased((nn::hid::NpadButton)nx_k))
                io.AddKeyEvent((ImGuiKey)im_k, false);
        }

        Starlight::HID::getMouseCoords(&io.MousePos.x, &io.MousePos.y);

        ImVec2 scrollDelta(0, 0);

        Starlight::HID::getScrollDelta(&scrollDelta.x, &scrollDelta.y);
        ...
        {
            if (Starlight::HID::isMousePressed((nn::hid::MouseButton)nx_k))
                io.AddMouseButtonEvent((ImGuiMouseButton)im_k, true);
            else if (Starlight::HID::isMouseReleased((nn::hid::MouseButton)nx_k))
                io.AddMouseButtonEvent((ImGuiMouseButton)im_k, false);
        }

        for (auto [im_k, nx_k] : key_mapping)
        {
            if (Starlight::HID::isKeyPressed((nn::hid::KeyboardKey)nx_k))
            {
                io.AddKeyEvent((ImGuiKey)im_k, true);
            }
            else if (Starlight::HID::isKeyReleased((nn::hid::KeyboardKey)nx_k))
            {
                io.AddKeyEvent((ImGuiKey)im_k, false);
            }
        }
